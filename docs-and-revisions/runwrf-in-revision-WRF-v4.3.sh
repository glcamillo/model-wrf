...
#
#  Versão deste script
#         0.1 (2009): foram feitas algumas tentativas para criar este script - sem sucesso.
...
### TODO em revisão e atualização para a nova versão 4.3 do WRF
#   12out21: INÍCIO: atualização para a v4.3 do WRF (incluindo os scripts auxiliares)

# ----------------------------------------
# Gratitude: special thanks to the people that worked hard to open new roads. Vide Readme.

## TODO  permanente
#  Revisar o presente e os demais quanto à legibilidade, segurança e resiliência (no que for possível).


## TODO TODO TODO
#  Definir parâmetro de resol dados globais com base na grade escolhida (padrão). E, deixar, a escolha via parâmetro de linha de comando como uma opção para sobreescrever a padrão.
#  Criar uma estrutura de diretórios para dados globais de entrada e dados intermediários e de saída
#  model-data-input-global
#     yyyy-mm-dd-HH-gfs    e  yyyy-mm-dd-HH-cptec-wrf
#  model-data-output
#     yyyy-mm-dd-HH-domain-A-SC    yyyy-mm-dd-HH-domain-F-SP
...
function ajuda
{
	echo " Uso: \$ $(basename "$0") PARÂMETROS [OPÇÕES]"
#	echo " -i no-ifrest|ifrest : no-ifrest (primeira rodada) ifrest (proximas rodadas)"
    echo " -d_conf A | B | ... Z: configuração de domínio"
...
#	echo " -tr mes|dia : duracao temporal da rodada (NAO IMPLEMENTADO) - padrao:mes"
	...
	echo " -g gfs1p00 | gfs0p50 | gfs0p25 | cptec_wrf_5km : dados globais NCEP (1|0,5|0,25 graus - padrão:gfs1p00) ou WRF do CPTEC (1km)"
...
{
    if [ x$1 = x"gfs1p00" ]; then 
        GLOBAL_DATA="gfs1p00"
    elif [ x$1 = x"gfs0p50" ]; then  
        GLOBAL_DATA="gfs0p50"
    elif [ x$1 = x"gfs0p25" ]; then
         GLOBAL_DATA="gfs0p25"
    elif [ x$1 = x"cptec_wrf_5km" ]; then
        GLOBAL_DATA="cptec_wrf_5km"  
    fi
...
# Em 20out2021: função para definições de temporais
function t_resolucao_temporal
{

    # Variável para controlar o período dos dados globais
    INCR=6
    
    
    # Variável para controlar o período de integração.
    #   Intervalo em segundos (namelist interval_seconds)
    export WRF_TIME_STEP=21600    # 21600s=6h
...
# Em 20out2021: opções de física
function wrf_opcoes_fisica
{
...
# 	      x) shutdown_execution "Saindo ..." 0;;
# 	      *) shutdown_execution "Opcao desconhecida. Saindo ..." 1 ;;
#       esac
...
#       else shutdown_execution "Resposta fora das opcoes disponiveis. 1"
#       fi
#       export _CUMULUS_D1
# 
...
#                                ;;
        -ti |  --t_integracao ) shift
                                ...
                                ;;
        -g | --f_global_data )   	shift
                                parametro=$1
				                f_global_data $parametro			
                                ;;
        -np | --numproces )   	shift
                                export NUM_PROC=$1
                                ;;
        ...
        -h | --help )           ajuda
                                ...
                                ;;
        * )                     ajuda
                                shutdown_execution " ERRO: faltam parametros. Saindo ...  " 1
    esac
...
if [ $NUM_PARAM -ne 3 ]; then
    ...
    ajuda
#	shutdown_execution "  ERRO: falta(m) parametro(s). Saindo ..." 1
fi
...
let WRF_TIME_STEP_HORAS=$WRF_TIME_STEP/3600
echo "WRF_TIME_STEP: $WRF_TIME_STEP(s)  - $WRF_TIME_STEP_HORAS(h)" >> wrf.log
...
DIR_DADOS_GFS=$USER_PATH/model-data-input-global

gfs-data-$DATA$HORA_INICIAL'Z'
...
# TODO TODO TODO
# result=$(fetch_global_data  ${DIR} ${DATA}${HORA_INICIAL} ${GLOBAL_DATA_TIME_INTERVAL} ${WRF_TIME_STEP} $GLOBAL_DATA)

result=$(fetch_global_data  ${DIR} ${DATA}${HORA_INICIAL} ${GLOBAL_DATA_TIME_INTERVAL} $GLOBAL_DATA)
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#
#   Function to found the next round in the same configuration.
#
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
[aA]) # Configuração com tres dominios 
        ...
        # 20211223: conf determines the choice of global data (source and/or resolution.)
        GLOBAL_DATA=GFS_1p00   # GFS_0p50  GFS_0p25   NCEP_WRF_1km
...
# Execution of the script to generate the namelist.wps with values
#   obtained from exported variables.
result=($(WPS_PATH)/namelist.wps.sh  WPS  $(WPS_PATH)/data/FILE)
if test result -ne 0; then
    shutdown_execution  "ERROR: problem in execution of $(WPS_PATH)/namelist.wps.sh. Exiting."  1
...
if [ $status -ne 0 ]; then
    ...
    ./processa_ungrib.sh  $GLOBAL_DATA
    retorno=$(echo $?)
...
if [ $status -ne 0 ]; then
    ...
    if test $retorno -ne 0 ; then
        # Remove OUTPUT data from metgrid
        # rm -f wrfinput_d0? > /dev/null
        shutdown_execution "ERROR: Problem in the execution of WPS/metgrid/metgrid.exe. Leaving ..." 1
    fi
fi
...
#     Input data (files): met_em.d01.$INICIO_ANO-$INICIO_MES-$INICIO_
...
if [ $status -ne 0 ]; then
    ...
    result=$(echo $?)    
    if test $result -ne 0 ; then
        # Remove OUTPUT data from real.exe    
        rm -f wrfinput_d0? > /dev/null
        rm -f wrfbdy_d0? > /dev/null 
    ...
    if test ! -e  wrfinput_d01 -o test ! -e  wrfbdy_d01; then
        # Remove OUTPUT data from real.exe
        rm -f wrfinput_d0? > /dev/null
        rm -f wrfbdy_d0? > /dev/null 
        shutdown_execution "ERROR: Problem in execution of real.exe. Exiting ..." 1
    ...
    # Check the number of TIMES present in each output file. 
    # TODO TODO Not satisfatory. TO REVIEW.
    result=$(ncdump -v Times wrfbdy_d01 | grep Time | head -n 1 | egrep '[0-9]' | tr -s ' ' | cut -d '(' -f2 | cut -d ' ' -f 1)
...
if [ $status -ne 0 ]; then
    ...
    if [ -e wrfout_d01_$(INICIO_ANO)-$(INICIO_MES)-$(INICIO_DIA)'_'$(INICIO_HORA):00:00 ] || [ -e wrfout_d02_$(INICIO_ANO)-$(INICIO_MES)-$(INICIO_DIA)'_'$(INICIO_HORA):00:00 ] || [ -e wrfout_d03_$(INICIO_ANO)-$(INICIO_MES)-$(INICIO_DIA)'_'$(INICIO_HORA):00:00 ]; then
        ...
        # Remove INPUT data from real.exe
        rm -f wrfinput_d0? > /dev/null
        rm -f wrfbdy_d0? > /dev/null 
    else
        # Remove OUTPUT data from wrf.exe
        rm -f wrfout_d0?_* > /dev/null
        shutdown_execution "ERROR: Problem in execution of wrf.exe. Exiting ..." 1
