...
# Codif: UTF-8
#
#  namelist.input.wrf.sh : cria o Namelist para o WRF
...
### TODO TODO em revisão e atualização para a nova versão
...
! Spatial resolutions of nests are calculated from parent_grid_ratio. No more necessary.
...
! The ratio of time step can be different from grid ratio.
 parent_time_step_ratio              = 1,     3,     3,
! Parameter that control the update from nest do the mother domain (0=no feedback; 1=one-way;.
 feedback                            = $(_FEEDBACK),  ! feedback from nest to its parent domain; 0 = no feedback
! Smooth option: 0 (no smoothing) a 2 (smoothing-desmoothing (default))
 ...
 bldt                                = 0,     0,     0,
 cu_physics                          = $(_CU_PHYSICS_1), $(_CU_PHYSICS_1), $(_CU_PHYSICS_1),  ! Cumulus scheme
 cudt                                = 5,     5,     5,
 isfflx                              = 1,
 ifsnow                              = 0,
 icloud                              = 1,
 ...
 num_soil_layers                     = 4,
 num_land_cat                        = 21,   ! number of land categories in input data.For MODIS if including lake category (default since 3.8)
