...
* WRF: ./modelo/wrf/grads/plot-sigma-wrf.gs
...
*  17jun21: algumas alterações nas variáveis para adequar às saídas wrfout_d01_SIG

* #TODO: há necessidade de refazer
...
* gradsc -blc "run plot-sigma.gs wrfout_d01_SIG.ctl y $MKX"
...
*** NOVOS ACRÉSCIMOS CONSIDERANDO O WRF
...
*************************************************
*  AGUA PRECIPITAVEL (em mm) e CAPE- calculado pelo GraDs
*************************************************
i=1
while (i < nPTempo)
  'clear'
  ...
  'set cmin 6' 
  'set cmax 60'
  'set clevs  6 12 18 24 30 36 42 48 54 60'
  'set ccols 67 69 71 73 75 77 79 81 83 85'
  'd mcape'
  'run rgbset.gs'
  ...
  'draw title Agua Precipitavel (mm) e CAPE (J/kg) - ' data ' ' hora
  'printim agua-prp-cape-' validade.i '.png ' x_pontos ' ' y_pontos ' white'
  i = i + 1
endwhile
...
*  'set cint 2'
  ...
  'draw title CIN integrado (J/kg) - ' data ' ' hora
  'printim mcin-' validade.i '.png ' x_pontos ' ' y_pontos ' white'
  i = i + 1
endwhile
...
while (i < nPTempo)
   ...
   if (i = 1)
       ...
       'printim prp-rainc-' validade.i '.png ' x_pontos ' ' y_pontos ' white'
   else
       'set gxout shaded'
       'set cmin 0.1'
       'set cmax 50'
...
while (i < nPTempo)
   ...
   if (i = 1)
       ...
       'd slp'
       'draw title Pressao Superficie ' data' 'hora
       'printim prp-rainsh-' validade.i '.png ' x_pontos ' ' y_pontos ' white'
   else
       'set gxout shaded'
       'set cmin 0.009'
       'set cmax 50'
